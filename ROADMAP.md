...
| G98 | a Gherkin edit's cursor-follow used a plain `text in new_texts` check -- wrong whenever the current step's exact text repeats elsewhere in the same scenario (`.index()` always resolves to the FIRST occurrence, not the one the cursor was actually on), and a reworded step fell back to a raw index clamp that ignores every OTHER shift in the same edit. Neither case was surfaced live -- only a nested JSON field a caller had to remember to check | Real `difflib.SequenceMatcher` diff of old/new step text instead: matches by position within the matching block (fixes the duplicate-text case), and a reworded step anchors to where its block now starts, accounting for every other change in the same edit (`"relocated"`, distinct from a plain `"clamped"`). Either non-clean case now also prints a loud live warning regardless of pass/fail, not just a field to go check. |
| G99 | `cdp list`/`cdp status`/`debug list`/`debug status` run one directory too high (a monorepo root, say) silently reported empty/"no session" -- indistinguishable from genuinely nothing tracked, since `find_and_load`'s bare fallback (no aitlc.toml found) also silently defaulted `root_dir` to the wrong directory (`Path.cwd()`, ignoring the directory it was actually told to search from) | The bare fallback now records that no config was found and uses the directory actually searched from as `root_dir`; the four commands above attach a `"warning"` naming that directory instead of reporting an indistinguishable empty result. |
| G100 | `cdp stop`/`cdp stop --all` killed the browser a live `debug` session was actively using, with no warning at all -- `debug status`/`continue` kept reporting success afterward, since a step that never touches the page (a pure filesystem assertion, say) genuinely can pass with no browser at all, silently masking that the session's browser was gone | Either form now names any debug session(s) whose port was just stopped in a `"warning"`, so orphaning one is a visible, deliberate moment -- not a surprise discovered several steps later. |
| G101 | plain `aitlc run` dumped every step's full record on stdout even on a CLEAN PASS with zero failures -- redundant with behave's own human-readable summary printed just above it in the same output, real complaint hit live on a 37-step scenario that fully passed | Same contract G86 already gave `debug continue`: `[run].output`/`run --full`/`--compact` (compact default) drops the per-step `"steps"` array when the run passed with zero failures; any failure keeps it regardless. The journal always keeps the complete record either way. |
| G102 | no way to look up a step's real info (pattern, description, usage, API flag) from the terminal without opening StepAtlas's site, and no single command to build/serve it | `aitlc stepatlas build`/`serve`/`info <query>` -- a thin wrapper: shells out to StepAtlas's own CLI/site and reads its `catalog.json` directly, no reimplementation. `info` matches by text fragment or `file:line` (nearest-before fallback). New `[stepatlas].path` config. |
